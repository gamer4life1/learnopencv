...
def surgery():
    ...
    original_image = cv2.resize(
        original_image, None, None, fx=1 / 2.0, fy=1 / 2.0)
    ...
    with torch.no_grad():
        ...
        score_map = preds[0, class_idx[0, col_idx,
                                       row_idx[0, col_idx]], :, :].cpu().numpy()
        ...
        cv2.imshow("activations", (original_image *
                                   score_map).astype(np.uint8))
        cv2.waitKey(0)
        cv2.imwrite("activations_with_pooling.png",
