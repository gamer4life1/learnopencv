// This code is written by Sunita Nayak at BigVision LLC. It is based on the
// OpenCV project. It is subject to the license terms in the LICENSE file found
// in this distribution and at http://opencv.org/license.html
...
int main(int argc, char **argv) {
  ...
  parser.about(
      "Use this script to do Augmented Reality using Aruco markers in OpenCV.");
  if (parser.has("help")) {
    parser.printMessage();
    return 0;
  }
  ...
  if (!parser.has("image")) {
    video.open(outputFile, VideoWriter::fourcc('M', 'J', 'P', 'G'), 28,
               Size(2 * cap.get(CAP_PROP_FRAME_WIDTH),
  ...
  while (waitKey(1) < 0) {
    ...
    try {
      ...
      // Using the detected markers, locate the quadrilateral on the target
      // frame where the new scene is going to be displayed.
      ...
      std::vector<int>::iterator it =
          std::find(markerIds.begin(), markerIds.end(), 25);
      ...
      pts_dst.push_back(Point(refPt1.x - round(scalingFac * distance),
                              refPt1.y - round(scalingFac * distance)));

      pts_dst.push_back(Point(refPt2.x + round(scalingFac * distance),
                              refPt2.y - round(scalingFac * distance)));
      ...
      pts_dst.push_back(Point(refPt3.x + round(scalingFac * distance),
                              refPt3.y + round(scalingFac * distance)));
      ...
      pts_dst.push_back(Point(refPt4.x - round(scalingFac * distance),
                              refPt4.y + round(scalingFac * distance)));
      ...
      // Prepare a mask representing region to copy from the warped image into
      // the original frame.
